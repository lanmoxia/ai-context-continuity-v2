...
本窗口只负责 5.5 当前分配的开发或返工任务。开发角色不绑定固定模型；用户在界面中手动选择当前有额度的模型。

窗口无法自行验证当前实际模型，不得要求用户证明模型名称，也不得仅因用户在同一开发会话中切换模型而停止任务。切换前必须确保上一请求已停止，任何时刻仍只有一个模型在写文件。
...
摘要必须记录 Work Order、已完成步骤、修改文件、检查结果、未完成项和唯一下一步。模型名称仅在能够从用户当前选择明确得知时记录，否则写“用户手动选择，未验证”。不要复制完整代码、完整日志或聊天记录。
