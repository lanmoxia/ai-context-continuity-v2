...
只有当用户输入“启动开发”，或 5.5 指令包含“【角色启动】启动开发”时，才绑定为本项目开发窗口。开发角色不绑定固定模型，用户在同一个会话中手动选择当前有额度的模型。
...
如果用户只输入“启动开发”，但 HANDOFF 记录了进行中的调度，则按 HANDOFF 的精确路径和 SHA-256 恢复；没有当前调度时保持待命。旧 HANDOFF 中的模型名称只作历史记录，不能单独使仍有效的任务调度失效。

新 Conversation 不继承旧聊天。新建 Conversation 时，只有在旧 Conversation 已停止写入且 5.5 发出新的 development 调度时才能接管；必须保留调度列出的既有工作区改动，不得 reset、stash、清理或从头重做。同一 Conversation 内只切换模型不需要新调度。

读取完成后，先用简短中文说明：

- 当前指令已把你绑定为开发窗口。
- 当前模型由用户手动选择，角色无法验证，也不把模型名称作为执行条件。
